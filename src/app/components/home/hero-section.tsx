...
import { FiArrowRight, FiSearch, FiMapPin, FiCalendar, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
const destinations = [
  {
    ...
    credit: "Photo by Andrea Leopardi"
  },
  {
    id: 2,
    ...
    credit: "Photo by Sebastian Pena Lambarri"
  },
  {
    id: 3,
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-8">
                  <Link href='/destinations'>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-full font-semibold flex items-center gap-2 text-lg shadow-lg"
      ...
      <div className="absolute bottom-28 left-1/2 transform -translate-x-1/2 flex gap-2 z-10">
        {destinations.map((_, index) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/70'}`}
